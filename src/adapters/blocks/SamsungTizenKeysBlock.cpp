...
constexpr BlockSchema SCH = {
    "samsung-key", "Samsung TV: send key", "TV (Samsung)", FIELDS, 3, nullptr, 0,
};
...
TvController& tv()       { static TvController t; return t; }
String&       tvIp()     { static String s;        return s; }
uint32_t&     tvLastMs() { static uint32_t v = 0;  return v; }
...
public:
    ...
    RunResult run(JsonVariantConst params,
        ...
        bool needConnect = (tvIp() != String(ip)) ||
                           (tvIp().length() == 0) ||
...
SamsungKeyBlock& instance(){ static SamsungKeyBlock i; return i; }
